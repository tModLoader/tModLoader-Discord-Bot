...
namespace tModloaderDiscordBot.Services
{
	public class GuildTagService : BaseConfigService
	{
		...
		public IEnumerable<GuildTag> GuildTags 
			=> _guildConfig?.GuildTags ?? Enumerable.Empty<GuildTag>();
		private IEnumerable<GuildTag> GuildTagsOwnedBy(ulong id) 
			=> GuildTags.Where(x => x.IsOwner(id));
		public bool HasTag(string key) 
			=> _guildConfig.GuildTags.Any(x => x.MatchesName(key));
		public bool HasTag(ulong id, string key) 
			=> GuildTagsOwnedBy(id).Any(x => x.MatchesName(key));
		public GuildTag GetTag(ulong id, string key) 
			=> GuildTagsOwnedBy(id).FirstOrDefault(x => x.MatchesName(key));
		public IEnumerable<GuildTag> GetTags(ulong id) 
			=> GuildTagsOwnedBy(id);
		public IEnumerable<GuildTag> GetTags(string predicate, ulong? id = null, bool globalTagsOnly = false)
		{
			...
			{
				if (globalTagsOnly && !guildTag.IsGlobal) 
					continue;
